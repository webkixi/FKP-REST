...
module.exports = {
        ...
        if (entrys) {
            ...
            function doScript(options) {
                ...
                if (options.method === 'gulp' || nowp.ie || nowp.common) {
                    var _dist = options.env === 'pro' ? path.join(__dirname, '../../', config.dist + '/' + configs.version + '/js/') : path.join(__dirname, '../../', config.dist + '/' + configs.version + '/dev/js/')
    ...
    readDirs: function(dirname, isPack, options) {
        ...
        if (typeof dirname === 'string') {
            var _tmp = this.readDir(dirname, isPack, options);
            return _tmp
        } else
        if (getObjType(dirname) === 'Array') {
