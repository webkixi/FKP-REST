...
}else{return false}},setter:function(name,dataOrAct,fun){if(!name||name==""){return false}var save=_stock;if(!save[name]){var thisStore=new store();save[name]=thisStore}if(dataOrAct&&dataOrAct!==""){if(getObjType(dataOrAct)==="Function"){if(getObjType(fun)==="Array"){dataOrAct.args=fun}save[name].acter(dataOrAct)}else{if(getObjType(dataOrAct)==="Object"||getObjType(dataOrAct)==="String"||getObjType(dataOrAct)==="Boolean"){save[name].dataer(dataOrAct)}else{if(getObjType(dataOrAct)==="Array"){var isFuns=true;dataOrAct.map(function(item,i){if(getObjType(item)!=="Function"){isFuns=false}});if(isFuns){if(getObjType(fun)==="Array"){dataOrAct.map(function(item,i){if(getObjType(fun[i])==="Array"){item.args=fun[i]}else{item.args=[fun[i]]}})}save[name].sact=dataOrAct}else{save[name].dataer(dataOrAct)}}}}}if(getObjType(fun)==="Function"){save[name].acter(fun)}if(getObjType(fun)==="Array"){var isFuns=true;fun.map(function(item,i){if(getObjType(item)!=="Function"){isFuns=false}});if(isFuns){save[name].sact=fun}}if(getObjType(fun)==="Object"){if(save[name].sact){var sact=save[name].sact;if(getObjType(sact)==="Array"){if(!sact.length){sact={}}else{console.log("SA set error, fun is array ");return false}}if(getObjType(sact)==="Object"){var target=extend(sact,fun);save[name].sact=target}}else{save[name].sact=fun}}},getter:function(name){if(!name||name==""){return}var save=_stock;if(save[name]){var that=save[name];function runner(data,key){that.dataer(data,key)}return{run:runner,data:save[name].getter("data"),action:save[name].getter("action")}}else{return false}},deleter:function(name){if(!name||name==""){return}var save=_stock;if(save[name]){delete save[name]}},runner:function(name,ddd,key){if(!name||name==""){return}var save=_stock;if(save[name]){var that=save[name];function runner(data,key){that.dataer(data,key)}var _data=that.getter("data");if(ddd&&getObjType(ddd)==="Object"){_data=extend(true,_data,ddd)}runner(_data,key)}},lister:function(){return Object.keys(_stock)}};window.SA=storeAct})();
